...
async function getEndeavorColor(endeavor) {
    const endeavors = await getEndeavors();
    const foundEndeavor = endeavors.find(e => e.name === endeavor);
    return foundEndeavor ? foundEndeavor.color : '#039BE5';
}

async function processEvents(observer) {
    // Disconnect the observer to prevent infinite loop
    if (observer) observer.disconnect();

    const events = document.querySelectorAll(task_selector);
    ...
    events.forEach(async event => {
        ...
        if (titleElement) {
            ...
            if (endeavor) {
                let task_body = event.closest('.KF4T6b');
                if (task_body) {
                    let color = await getEndeavorColor(endeavor);
                    task_body.style.backgroundColor = color;
                }
            }
        }
    });
